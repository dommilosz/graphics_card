...
void VgaInit(const sVmode* vmode)
{
	...
	for (i = 0; i < 256; i++)
	{
		...
		u32 m = 0;
		if ((i & _1LS7) != 0) m |= 0xff;
		if ((i & _1LS6) != 0) m |= 0xff << 8;
		if ((i & _1LS5) != 0) m |= 0xff << 16;
		if ((i & _1LS4) != 0) m |= 0xff << 24;
		RenderTextMask[2*i] = m;

		// lower 4 bits
		m = 0;
		if ((i & _1LS3) != 0) m |= 0xff;
		if ((i & _1LS2) != 0) m |= 0xff << 8;
		if ((i & _1LS1) != 0) m |= 0xff << 16;
		if ((i & _1LS0) != 0) m |= 0xff << 24;
...
//	Frame = 0;
	...
	LayerMask = _1LS0; // mask of active layers
	for (i = 1; i < LAYERS; i++)
	{
